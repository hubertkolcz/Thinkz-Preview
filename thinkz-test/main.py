    ...
    if request.method == "POST":
        ...
        if (eq_first == "" or eq_first == "/" or eq_first == 0) and eq_last == "":
            ...
            try:
                ...
                # result2_bool = (solveset(snumber3, x) == solveset(snumber4, x))
                # tester = str(result2_bool)
                # if len(solveset(snumber3,x)) == 2:
                #     try:
                #         score = "First score is {} and second is {}".format(solve(snumber3,x)[0] == float(expr(raw(eq_final_1))),solve(snumber3,x)[1] == float(expr(raw(eq_final_2))))
                #     except:
                #         score = "Error"
                # else:
                #     try:
                #         score = solve(snumber3,x)[0] == float(expr(raw(eq_final_1)))
                #     except:
                #         score = "Error"
